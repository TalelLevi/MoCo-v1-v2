...
import torchvision
from src.base_classes.abstract_model import Model
...
class MoCo_v2(Model):
    ...
    def __init__(self,
        ...
        self.register_buffer("queue",
                             nn.functional.normalize(torch.randn(self.queue_size, self.dim, requires_grad=False),
                                                     dim=1) / 10)
        self.ptr = 0
        ...
        if self.mlp:
            self.q_encoder.fc = nn.Sequential(
                nn.Linear(self.q_encoder.fc.weight.shape[1], self.q_encoder.fc.weight.shape[1]),
                nn.ReLU(),
                nn.Linear(self.q_encoder.fc.weight.shape[1], self.dim, bias=self.bias))
            self.k_encoder.fc = nn.Sequential(
                nn.Linear(self.k_encoder.fc.weight.shape[1], self.k_encoder.fc.weight.shape[1]),
                nn.ReLU(),
    ...
    def moco_forward(self, x, prints=False):
        """ moco phase forward pass """
        q, k = x  # TODO check this
